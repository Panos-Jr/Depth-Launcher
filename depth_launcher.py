...
POLL_INTERVAL = 3
CLIENT_BAT    = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "client.bat")
...
class App(tk.Tk):
    def __init__(self):
        ...
        icon = tk.PhotoImage(file=os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "steering-wheel.png"))
    ...
    def _build_ui(self):
        ...
        icon_img = tk.PhotoImage(file=os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "steering-wheel.png"))
        icon_img = icon_img.subsample(20, 20)
        self._icon_img = icon_img

        tk.Label(header_frame, image=icon_img, bg="#0d1117").pack(side="left", padx=(0, 6))
    ...
    def _do_launch(self):
        ...
        try:
            ...
            self.after(0, lambda: self._set_state("error", f"Failed to launch game: {e}"))
